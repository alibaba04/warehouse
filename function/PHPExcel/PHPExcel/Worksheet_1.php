...
/**
	...
	*/
	public function hasCodeName(){
        ...
        public function setTableHeader($colTitle, $rowStart, $sheet)
        {            
            foreach ($colTitle as $col=>$title)
            {
                $sheet->setCellValueByColumnAndRow($col, 4, $title);
                $sheet->getColumnDimension(PHPExcel_Cell::stringFromColumnIndex($col))->setAutoSize(true);
            } 
            $sheet->getStyle(PHPExcel_Cell::stringFromColumnIndex(0).$rowStart.":".PHPExcel_Cell::stringFromColumnIndex(count($colTitle)-1).$rowStart)->getFont()->setBold(true);
            $sheet->getStyle(PHPExcel_Cell::stringFromColumnIndex(0).$rowStart.":".PHPExcel_Cell::stringFromColumnIndex(count($colTitle)-1).$rowStart)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        }
}
